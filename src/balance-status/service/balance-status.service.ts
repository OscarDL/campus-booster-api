...
import { 
    BalanceStatusModel, 
    BalanceStatusAttributes, 
    BalanceStatusScopesAttributes,
    BalanceStatusCreationAttributes
} from '../model/balance-status.interface';
import { models } from '../../../config/models.config'; 
const { BalanceStatus } = models;
/**
...
export function findAll(
    options?: FindOptions<BalanceStatusAttributes> | null,
    scope?: (BalanceStatusScopesAttributes | BalanceStatusScopesAttributes[])
): Promise<BalanceStatusModel[]> {
    return BalanceStatus.scope(scope).findAll(options!);
}
/**
...
export function findOne(
    options?: FindOptions<BalanceStatusAttributes> | null,
    scope?: (BalanceStatusScopesAttributes | BalanceStatusScopesAttributes[])
): Promise<BalanceStatusModel | null> {
    return BalanceStatus.scope(scope).findOne(options!);
}
/**
...
export function findById(
    ...
    return BalanceStatus.scope(scope).findByPk(id, options!);
}
/**
...
export function create(
    data: BalanceStatusCreationAttributes,
    scope?: (BalanceStatusScopesAttributes | BalanceStatusScopesAttributes[])
): Promise<BalanceStatusModel> {
    return BalanceStatus.scope(scope).create(data);
}
/**
...
export function createMany(
    ...
    return BalanceStatus.scope(scope).bulkCreate(data);
}
/**
...
export function update(
    id: number | any, 
    data: Partial<BalanceStatusAttributes>,
    scope?: (BalanceStatusScopesAttributes | BalanceStatusScopesAttributes[])
): Promise<BalanceStatusModel> {
    return new Promise((resolve, reject) => {
        BalanceStatus.scope(scope).findByPk(id)
        .then((balancestatus) => {
            if (!balancestatus) {
                reject("not found");
            } else {
...
export function updateMany(
    options: UpdateOptions<BalanceStatusAttributes>, 
    data: Partial<BalanceStatusAttributes>,
    scope?: (BalanceStatusScopesAttributes | BalanceStatusScopesAttributes[])
): Promise<[affectedCount: number]> {
    return BalanceStatus.scope(scope).update(data, options);
}
/**
...
export function remove(
    ...
    return BalanceStatus.destroy(options!);
}
/**
...
export function count(
    ...
    return BalanceStatus.count(options!);
}
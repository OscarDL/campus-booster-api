    ...
    return async(req: Req, res: Res, next: Next): Promise<Resp> => {
        try {
            if(req.query[name]) {
                ...
                return (!userhasclassroom) ? next(boom.notFound(`resource_not_found`, [ "UserHasClassroom", req.query[name]])) : next();
            }
            return next();
        } catch (err: any) {
    ...
    return async(req: Req, res: Res, next: Next): Promise<Resp> => {
        try {
            if(req.body[name]) {
                ...
                return (!userhasclassroom) ? next(boom.notFound(`resource_not_found`, [ "UserHasClassroom", req.body[name]])) : next();
            }
            return next();
        } catch (err: any) {
            console.log(`${err}`.red.bold);
    ...
    return async(req: Req, res: Res, next: Next): Promise<Resp> => {
        try {
            if(req.params[name]) { 
                ...
                return (!userhasclassroom) ? next(boom.notFound(`resource_not_found`, [ "UserHasClassroom", req.params[name]])) : next();
            }
            return next();
        } catch (err: any) {
            console.log(`${err}`.red.bold);

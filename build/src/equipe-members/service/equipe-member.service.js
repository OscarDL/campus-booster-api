...
const { EquipeMember } = models_config_1.models;
/**
...
function findAll(options, scope = 'defaultScope') {
    return EquipeMember.scope(scope).findAll(options);
}
...
function findOne(options, scope = 'defaultScope') {
    return EquipeMember.scope(scope).findOne(options);
}
...
function findById(id, options, scope = 'defaultScope') {
    return EquipeMember.scope(scope).findByPk(id, options);
}
...
function create(data, scope = 'defaultScope') {
    return EquipeMember.scope(scope).create(data);
}
...
function update(id, data, scope = 'defaultScope') {
    return new Promise((resolve, reject) => {
        EquipeMember.scope(scope).findByPk(id)
            .then((equipemember) => {
            if (!equipemember) {
                reject("not found");
            }
...
function remove(options) {
    return EquipeMember.destroy(options);
}
...
function count(options) {
    return EquipeMember.count(options);
}
exports.count = count;

...
const { Contrat } = models_config_1.models;
/**
...
function findAll(options, scope = 'defaultScope') {
    return Contrat.scope(scope).findAll(options);
}
...
function findOne(options, scope = 'defaultScope') {
    return Contrat.scope(scope).findOne(options);
}
...
function findById(id, options, scope = 'defaultScope') {
    return Contrat.scope(scope).findByPk(id, options);
}
...
function create(data, scope = 'defaultScope') {
    return Contrat.scope(scope).create(data);
}
...
function update(id, data, scope = 'defaultScope') {
    return new Promise((resolve, reject) => {
        Contrat.scope(scope).findByPk(id)
            .then((contrat) => {
            if (!contrat) {
                reject("not found");
            }
...
function remove(options) {
    return Contrat.destroy(options);
}

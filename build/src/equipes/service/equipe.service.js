...
const { Equipe } = models_config_1.models;
/**
...
function findAll(options, scope = 'defaultScope') {
    return Equipe.scope(scope).findAll(options);
}
...
function findOne(options, scope = 'defaultScope') {
    return Equipe.scope(scope).findOne(options);
}
...
function findById(id, options, scope = 'defaultScope') {
    return Equipe.scope(scope).findByPk(id, options);
}
...
function create(data, scope = 'defaultScope') {
    return Equipe.scope(scope).create(data);
}
...
function update(id, data, scope = 'defaultScope') {
    return new Promise((resolve, reject) => {
        Equipe.scope(scope).findByPk(id)
            .then((equipe) => {
            if (!equipe) {
                reject("not found");
            }

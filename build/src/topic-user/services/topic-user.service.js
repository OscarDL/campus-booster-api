...
const { TopicUser } = models_config_1.models;
/**
...
function findAll(options, scope = 'defaultScope') {
    return TopicUser.scope(scope).findAll(options);
}
...
function findOne(options, scope = 'defaultScope') {
    return TopicUser.scope(scope).findOne(options);
}
...
function findById(id, options, scope = 'defaultScope') {
    return TopicUser.scope(scope).findByPk(id, options);
}
...
function create(data, scope = 'defaultScope') {
    return TopicUser.scope(scope).create(data);
}
...
function update(id, data, scope = 'defaultScope') {
    return new Promise((resolve, reject) => {
        TopicUser.scope(scope).findByPk(id)
            .then((topic) => {
            if (!topic) {
                reject("not found");
            }
...
function remove(options) {
    return TopicUser.destroy(options);
}
...
function count(options) {
    return TopicUser.count(options);
}
exports.count = count;

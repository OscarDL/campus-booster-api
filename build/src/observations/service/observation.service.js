...
const { Observation } = models_config_1.models;
/**
...
function findAll(options, scope = 'defaultScope') {
    return Observation.scope(scope).findAll(options);
}
...
function findOne(options, scope = 'defaultScope') {
    return Observation.scope(scope).findOne(options);
}
...
function findById(id, options, scope = 'defaultScope') {
    return Observation.scope(scope).findByPk(id, options);
}
...
function create(data, scope = 'defaultScope') {
    return Observation.scope(scope).create(data);
}
...
function update(id, data, scope = 'defaultScope') {
    return new Promise((resolve, reject) => {
        Observation.scope(scope).findByPk(id)
            .then((observation) => {
            if (!observation) {
                reject("not found");
            }
...
function remove(options) {
    return Observation.destroy(options);
}
...
function count(options) {
    return Observation.count(options);
}
exports.count = count;

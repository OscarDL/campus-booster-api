...
const { Option } = models_config_1.models;
/**
...
function findAll(options, scope = 'defaultScope') {
    return Option.scope(scope).findAll(options);
}
...
function findOne(options, scope = 'defaultScope') {
    return Option.scope(scope).findOne(options);
}
...
function findById(id, options, scope = 'defaultScope') {
    return Option.scope(scope).findByPk(id, options);
}
...
function create(data, scope = 'defaultScope') {
    return Option.scope(scope).create(data);
}
...
function update(id, data, scope = 'defaultScope') {
    return new Promise((resolve, reject) => {
        Option.scope(scope).findByPk(id)
            .then((option) => {
            if (!option) {
                reject("not found");
            }
